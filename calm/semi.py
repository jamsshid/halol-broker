"""
Semi Calm Mode - Risk Policy Framework
...
class SemiCalmMode:
    """
    Semi Calm mode risk policy.
    Configurable via settings, not hardcoded in trade logic.
    """
    
    # Risk limits for Semi Calm mode
    MAX_RISK_PERCENT = Decimal("2.0")  # 2% max risk per trade
    MAX_DAILY_LOSS_PERCENT = Decimal("5.0")  # 5% max daily loss
    MAX_POSITION_SIZE_PERCENT = Decimal("20.0")  # 20% max position size
    ...
    STRESS_FREE_MODE = False  # Show full PnL
    
    @classmethod
    def get_risk_config(cls) -> Dict[str, Any]:
        """
        Get risk configuration for Semi Calm mode.
    ...
    def validate_risk(cls, risk_percent: Decimal, position_size_percent: Decimal) -> bool:
        """
        Validate risk parameters against Semi Calm limits.
        ...
        if risk_percent > cls.MAX_RISK_PERCENT:
            raise ValueError(
                f"Semi Calm mode: Max risk is {cls.MAX_RISK_PERCENT}%, got {risk_percent}%"
            )
        
        if position_size_percent > cls.MAX_POSITION_SIZE_PERCENT:
            raise ValueError(
                f"Semi Calm mode: Max position size is {cls.MAX_POSITION_SIZE_PERCENT}%, got {position_size_percent}%"
            )
